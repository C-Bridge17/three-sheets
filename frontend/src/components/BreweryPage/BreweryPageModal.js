...
const BreweryPageModal = ({ el }) => {
  ...
  return (
    ...
    <div key={el.id} className="checkin-feed-div">
      ...
      <h3>
        <NavLink to={`/users/${el.User.id}`}>{el.User.username}</NavLink> {`is drinking ${a}`} <NavLink to={`/drinks/${el.Drink.id}`}>{el.Drink.name}</NavLink>
      </h3>
...
export default BreweryPageModal
