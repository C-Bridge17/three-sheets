...
const ProfileCheckin = ({ el }) => {
  ...
  return (
    ...
    <div key={el.id} className="checkin-feed-div">
      ...
      <h3>
        {`Drank ${a}`} <NavLink to={`/drinks/${el.Drink.id}`}>{el.Drink.name}</NavLink> by <NavLink to={`/stores/${el.Drink.Store.id}`}>{el.Drink.Store.title}</NavLink>
